"""
Train Origin-Destination (OD) Data Extraction Script

PURPOSE:
    This script fetches monthly train journey data from the LTA DataMall API.
    This is FACT data showing actual passenger journeys between train stations.
    
    Each record shows:
    - Origin station code (e.g., "NS1")
    - Destination station code (e.g., "EW12")
    - Time period (hour + weekday/weekend)
    - Number of trips (how many people made this journey)

WHY THIS MATTERS:
    Core transactional data for train analytics:
    - Understand MRT travel patterns
    - Identify busiest routes and interchange stations
    - Compare with bus data
...
IMPORTANT CONSTRAINTS:
    ...
    - Typically smaller than bus data (fewer stations = fewer OD pairs)
...
def download_and_extract_od_data(
    ...
    """
    Download train OD data for a specific month and extract the CSV.
    ...
    logger.info(f"Fetching train OD data for {year}-{month:02d}...")
    ...
    try:
        ...
        with LTAClient() as client:
            ...
            zip_content = client.get_train_od_data(year=year, month=month)
        ...
        output_filename = f"train_od_{year}{month:02d}.csv"
...
def print_summary(csv_path: Path) -> None:
    ...
    try:
        ...
        print("\n" + "="*70)
        print("TRAIN ORIGIN-DESTINATION DATA SUMMARY")
        print("="*70)
        ...
        if 'ORIGIN_PT_CODE' in df.columns and 'DESTINATION_PT_CODE' in df.columns:
            ...
            print(f"  Unique origin stations: {unique_origins:,}")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Extract train origin-destination data from LTA DataMall',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Extract data for January 2026
  python src/ingestion/extract_train_od.py --year 2026 --month 1
  
  # Extract data for previous month (default)
  python src/ingestion/extract_train_od.py
        """
    )
...
def main():
    """
    Main function - orchestrates the train OD data extraction.
    """
    ...
    logger.info("="*70)
    logger.info(f"TRAIN OD DATA EXTRACTION STARTING - {year}-{month:02d}")
    logger.info("="*70)
    ...
    try:
        ...
        # Success!
        logger.info("="*70)
        logger.info("TRAIN OD DATA EXTRACTION COMPLETED SUCCESSFULLY")
        logger.info("="*70)
        logger.info(f"Output file: {output_path}")
        
    except Exception as e:
        logger.error("="*70)
        logger.error("TRAIN OD DATA EXTRACTION FAILED")
        logger.error("="*70)
